...
\def\OuterSquarePad{0.075} % Distance between the outer square and the inner square
\def\OuterSquareLength{2}% Side length of the outer square is \OuterSquareLength+\OuterSquarePad
...
\begin{subfigure}{0.3\textwidth}%
	\centering\begin{tikzpicture}[x=2cm,y=2cm]
		\coordinate (circle-mid) at (\OuterSquareLength/2, \OuterSquareLength/2); % In retrospect, it would have been better to choose this midpoint as (0,0)
		% (0,0) is the lower left corner of the outer square if \OuterSquarePad=0, and otherwise the lower left corner is slightly below left of (0,0)
		% Outer square
		\draw[fill=hat-colour] (-\OuterSquarePad,-\OuterSquarePad) rectangle (\OuterSquareLength+\OuterSquarePad, \OuterSquareLength+\OuterSquarePad);
		% Inner square
		...
		% ---------- Dimensioning ----------
		% Outer square
		\draw[<->, shift={(-\OuterSquarePad,-\OuterSquarePad-\OuterSquareDimYShift)}] (0,0) -- (\OuterSquareLength+2*\OuterSquarePad,0) node[midway,below]{\OuterSquareLabel};
		% Inner square
...
\begin{subfigure}{0.33\textwidth}
	\centering\begin{tikzpicture}[x=2cm,y=2cm]%[line width=0.8pt]
		...
		% Coordinates as in the figure above
		% Inner square
